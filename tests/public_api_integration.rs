...
use loregrep::{
    LoreGrep, LoreGrepBuilder, LoreGrepError, Result, ScanResult, ToolResult, ToolSchema, VERSION,
};
use serde_json::json;
...
async fn test_execute_tool_interface() {
    ...
    let result = loregrep
        .execute_tool("get_repository_tree", json!({}))
        .await;
    assert!(result.is_ok());
    let tool_result = result.unwrap();
    assert!(tool_result.success);
...
"#,
    ...
    if tool_result.success {
        ...
        let data_keys: Vec<&str> = tool_result
            .data
            .as_object()
        ...
        assert!(
            !data_keys.is_empty(),
            "Successful analysis should return some data"
        );
    }
