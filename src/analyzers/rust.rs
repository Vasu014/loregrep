...
impl LanguageAnalyzer for RustAnalyzer {
    ...
    fn extract_functions(
        ...
        // Match both full function definitions (`function_item`) and bodiless
        // function declarations (`function_signature_item`, e.g. `extern fn f();`).
        let query_str = r#"
            [
            ...
            for capture in query_match.captures {
                ...
                match capture_name.as_str() {
                    ...
                    "function" => {
                        ...
                        function_sig.generics = self.extract_generics(&capture.node, source);
                                    ...
                                    if param_cursor.goto_first_child() {
                                        loop {
                                            ...
                                            if param_child.kind() == "self_parameter" {
                                                // Represent the receiver (`self`, `&self`,
                                                // `&mut self`) as a parameter named "self".
                                                function_sig.parameters.push(Parameter::new(
                                                    "self".to_string(),
                                                    String::new(),
                                                ));
                                            } else if param_child.kind() == "parameter" {
                                                let param_text = param_child
                                                    .utf8_text(source.as_bytes())
                                                    .unwrap_or("");
                                                if let Some((name_part, type_part)) =
                                                    param_text.split_once(':')
                                                {
                                                    ...
                                                    let param =
                                                        Parameter::new(clean_name, param_type)
            ...
            for capture in query_match.captures {
                ...
                match capture_name.as_str() {
                    ...
                    "tuple_fields" => {
                        ...
                        // In tree-sitter-rust there is no `ordered_field_declaration` node.
                        // Under `ordered_field_declaration_list`, each field is a type node
                        // (labelled with the `type` field name), optionally preceded by a
                        // `visibility_modifier` sibling for `pub` fields.
                        let mut child_cursor = capture.node.walk();
                        ...
                        if child_cursor.goto_first_child() {
                            loop {
                                ...
                                if child.kind() == "visibility_modifier" {
                                    pending_pub = true;
                                } else if child_cursor.field_name() == Some("type") {
                                    let field_type = child
                                        .utf8_text(source.as_bytes())
                                        .unwrap_or("")
                                        .to_string();
                                    let field =
                                        StructField::new(field_index.to_string(), field_type)
                                            .with_visibility(pending_pub);
                                    struct_sig.fields.push(field);
                                    field_index += 1;
                                    pending_pub = false;
                                }
    ...
    fn extract_with_fallback(&self, content: &str, file_path: &str) -> PartialAnalysis {
        let mut analysis =
            PartialAnalysis::new(file_path.to_string(), "rust".to_string()).with_fallback();
        ...
        if let Ok(struct_regex) = Regex::new(r"(?m)^\s*(pub\s+)?struct\s+(\w+)") {
            for caps in struct_regex.captures_iter(content) {
                if let Some(name_match) = caps.get(2) {
                    let mut struct_sig = StructSignature::new(
                        name_match.as_str().to_string(),
                        file_path.to_string(),
                    );
...
use self::child_module::ChildType;
        ...
        let crate_import = imports
            .iter()
            .find(|i| i.module_path.contains("crate"))
            .unwrap();
...
pub const MAX_USERS: usize = 1000;
        ...
        let analysis = analyzer
            .analyze_file(comprehensive_code, "user.rs")
            .await
            .expect("Analysis failed");
        ...
        let get_id_function = tree_node
            .functions
            .iter()
            .find(|f| f.name == "get_id")
            .unwrap();
        assert!(get_id_function.is_const);

        let generate_id_function = tree_node
            .functions
            .iter()
            .find(|f| f.name == "generate_id")
            .unwrap();
...
use super::config::Settings;
        ...
        if let Some(hashmap_import) = tree_node
            .imports
            .iter()
            .find(|i| i.module_path.contains("HashMap"))
        {
    ...
    input.to_uppercase()
        ...
        for function in &tree_node.functions {
            assert_eq!(
                ...
                "Function {} has incorrect file path",
                function.name
            );
        }
        ...
        for struct_def in &tree_node.structs {
            assert_eq!(
                struct_def.file_path, file_path,
                "Struct {} has incorrect file path",
                struct_def.name
            );
        }
        ...
        for import in &tree_node.imports {
            assert_eq!(
                ...
                "Import {} has incorrect file path",
                import.module_path
            );
        }
        ...
        for export in &tree_node.exports {
            assert_eq!(
                ...
                "Export {} has incorrect file path",
        ...
        for call in &tree_node.function_calls {
            assert_eq!(
                ...
                "Function call {} has incorrect file path",
                call.function_name
            );
        }
    }
